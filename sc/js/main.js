...
var org_track;
$(function() {
    URLToArray(location.href);
    SC.initialize({
        client_id: '2d7cfcb45f4ecfe720664a8fa6970db1',
    });
    var track_count = 0
    $.each(tracks_in_url, function(i, query) {
        if (i == 0) {
            org_track = query
        } else {
            embed_SC(query)
        ...
        if (i == 40) {
            ...
            track_count++
            $(".track_count").text(track_count - 1);
            $(".org_track").text(org_track);
            $("h1").show()
        }
    });
});
function embed_SC(query) {
    console.log(query);
    SC.get('/tracks', {
        q: query
    }).then(function(track) {
        track_url = track[0].permalink_url
        SC.oEmbed(track_url, {
            auto_play: false
        }).then(function(oEmbed) {
            $(".players").append(oEmbed.html);
        });
    });
}
function URLToArray(url) {
    var items = url.substring(url.indexOf('?') + 1).split('&');
    $.each(items, function(i, track) {
        track = decodeURIComponent(track)
        tracks_in_url.push(track);
    });
    console.log(tracks_in_url);
}